...
class OCWS_Admin_Columns
{
    ...
    private function __construct() {
        ...
        //error_log('before added restrict_manage_posts');
        //add_action( 'restrict_manage_posts', array($this, 'render_mode_buttons'), 1 );
        add_action( 'restrict_manage_posts', array($this, 'render_filter_set_by_date'), 3, 2 );
        add_action( 'restrict_manage_posts', array($this, 'render_filter_set_by_shipping_data'), 2, 2 );
        add_action( 'restrict_manage_posts', array($this, 'action_display_export_buttons'), 120, 2 );
        //error_log('after added restrict_manage_posts');
    ...
    public function action_display_export_buttons($post_type, $which) {
        //error_log('restrict_manage_posts callback');
    ...
    public function render_filter_set_by_date($post_type, $which) {
        //error_log('restrict_manage_posts callback');
        if('shop_order' != $post_type || !$this->is_shipping_and_pickup_method_filter_chosen()) {
            return;
        }
    ...
    public function render_filter_set_by_shipping_data($post_type, $which) {
        //error_log('restrict_manage_posts callback');
        if('shop_order' != $post_type || !$this->is_shipping_and_pickup_method_filter_chosen()) {
            return;
        }
