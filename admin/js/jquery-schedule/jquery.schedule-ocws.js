...
;(function ($, window, document, undefined) {
  ...
  var defaults = {
      ...
      onAfterRemovePeriod: function () {},
      onDragPeriod: function () {},
      onResizePeriod: function () {},
      onUpdatePeriod: function () {},
      onChangeMaxHour: function() {}
    },
    pluginName = 'jqs';
  ...
  $.extend(Plugin.prototype, {
    ...
    init: function () {
      ...
      if (this.settings.mode === 'edit') {
        ...
        $(this.element).on('change', '.jqs-day-timepicker', function () {
        //  $this.settings.onChangeMaxHour.call($this);
         // console.log('change');
         // console.log(this);
        });
      }
    ...
    create: function () {
      ...
      $('<table class="jqs-table"><tr class="jqs-tbody"></tr><tr class="jqs-table-footer"></tr></table>').appendTo($(this.element));

      for (var i = 0; i < this.settings.days; i++) {
        $('<td><div class="jqs-day"></div></td>').
          appendTo($('.jqs-table tr.jqs-tbody', this.element));
      }
      ...
      for (var k = 0; k < this.settings.days; k++) {
        $('<div class="jqs-grid-day">' + '<span class="jqs-grid-day-title">' + this.settings.daysList[k] + '</span>' + dayRemove + dayDuplicate + '</div>').
     ...
     * Update days titles
     */
    updateDaysList: function (args) {

      var daysList = args[1];
      this.updateSettingsDaysList(daysList);
    },

    updateSettingsDaysList: function(daysList) {
      var $this = this;
      var gridHead = $('.jqs-grid-head', this.element);
      var days = $('.jqs-grid-day-title', gridHead);
      for (var k = 0; k < days.length; k++) {
        var day = $(days[k]);
        day.text(k < daysList.length? daysList[k] : '-');
      }
    },
    ...
    sanitizePeriodData: function( period ) {
      var defaultData = this.settings.periodDefaultData;
      if (!$.isArray(defaultData)) {
        defaultData = [];
      }
      if (typeof period.data == 'undefined' || !$.isArray(period.data)) {
        return defaultData;
      }
      var res = [];
      for (var i=0; i < defaultData.length; i++) {
        var fields = $.extend({}, this.settings.periodDataFields, defaultData[i]);
        if (fields.name) {
          for (var k=0; k < period.data.length; k++) {
            if (period.data[k].name == fields.name) {
              res.push($.extend({}, fields, period.data[k]));
              break;
            }
          }
        }
    ...
    add: function (parent, position, height, options) {
      ...
      options.data = this.sanitizePeriodData(options);
      ...
      for (var i = 0; i < options.data.length; i++) {
        ...
        periodData +='<div style="display:none" class="jqs-period-data" data-name="' + options.data[i].name + '" data-type="' + options.data[i].type + '" data-val="' + options.data[i].value + '" data-title="' + options.data[i].title + '">'
            + '<strong>' + options.data[i].title + '</strong> ' + options.data[i].value + '</div>';
      }
      var periodTime = '<div class="jqs-period-time">' + this.periodInit(position, position + height) + '</div>';
      var period = $('<div class="jqs-period">' +
        '<div class="jqs-period-container">' + periodTime + periodTitle + periodData + periodRemove + periodDuplicate + '</div>' +
        '</div>').css({
        'top': position * this.periodPosition,
        'height': height * this.periodPosition
      ...
      if (this.settings.mode === 'edit') {
        ...
        period.draggable({
          ...
          stop: function (event, ui) {
            ...
            $this.settings.onDragPeriod.call(this, period, $this.element);
          }
        }).resizable({
          grid: [0, this.periodPosition],
          containment: 'parent',
          ...
          stop: function (event, ui) {
            ...
            $this.settings.onResizePeriod.call(this, period, $this.element);
          }
        });
    ...
    remove: function (period) {
      if (!this.settings.onRemovePeriod.call(this, period, this.element)) {
        period.remove();
        this.closeOptions();
        this.settings.onAfterRemovePeriod.call(this, this.element);
      }
    },
    ...
    openOptions: function (event, period) {
      ...
      var pDataElements = $('.jqs-period-data', period);
      var pData = '';

      for (var i=0; i < pDataElements.length; i++) {
        var elt = $(pDataElements[i]);
        pData += '<div class="jqs-options-data-container">' + '<label>' + elt.data('title') + '</label>';
        if (elt.data('type') && elt.data('type') == 'boolean') {
          pData += '<input type="checkbox" data-name="' + elt.data('name') + '"' + (elt.data('val') == true ? 'checked' : '') + ' value="' + elt.data('val') + '" class="jqs-options-data"></div>';
        }
        else {
          pData += '<input type="text" data-name="' + elt.data('name') + '" placeholder="' + elt.data('title') +
              '" value="' + elt.data('val') + '" class="jqs-options-data"></div>';
        }
      }
      ...
      $('<div class="jqs-options">' + time + titleInput + pData + colorInput + remove + duplicate + close + '</div>').css({
        top: top,
        left: left
      ...
      $('.jqs-options-title', this.element).keyup(function (e) {

        $('.jqs-period-title', period).text($(this).val());
        period.attr('title', $(this).val());

        $this.settings.onUpdatePeriod.call(this, period, $this.element);
      });

      $('.jqs-options-title', this.element).keydown(function (e) {
      ...
      $('.jqs-options-data', this.element).change(function (e) {

        var name = $(this).data('name');
        if ($(this).is(':checkbox')) {
          $('.jqs-period-data[data-name="' + name + '"]', period).data('val', $(this).is(':checked'));
        }
        else {
          $('.jqs-period-data[data-name="' + name + '"]', period).data('val', $(this).val());
        }

        $this.settings.onUpdatePeriod.call(this, period, $this.element);
      });

      $('.jqs-options-data', this.element).keyup(function (e) {
      ...
      $('.jqs-options-data', this.element).keydown(function (e) {
    ...
    export: function () {
      ...
      $('.jqs-day', $this.element).each(function (index, day) {
        ...
        max_hour = $('.jqs-day-timepicker-'+index, $this.element).val();
        filter_picker = $('.jqs-fillter-picker-' + index, $this.element).val();
        var periods = [];
        $('.jqs-period', day).each(function (index, period) {
          periods.push($this.periodData($(period)));
        });

        if ( $this.settings.type == 'weekly' ) {
          data.push({
            day: index,
            max_hour: max_hour,
          ...
          if (index < $this.settings.daysList.length && $this.settings.daysList[index] != '-') {
            data.push({
              day: $this.settings.daysList[index], // add the date
              max_hour: max_hour,
              filter_picker: filter_picker,
              periods: periods
            });
    ...
    addMaxHour: function () {
      ...
      /*for (var i = 0; i < this.settings.days; i++) {
        $('<td><div><select style="margin-bottom: 5px; width: 100%" class="jqs-fillter-picker jqs-fillter-picker-'+i+'">'+
            ...
            '</select></div><div style="height: 30px;"><input type="text" class="jqs-day-timepicker jqs-day-timepicker-'+i+'"></div></td>').
            appendTo($('.jqs-table tr.jqs-table-footer', this.element));
      }*/
      for (var i = 0; i < this.settings.days; i++) {
        $('<td><div><select style="margin-bottom: 5px; width: 100%" class="jqs-fillter-picker jqs-fillter-picker-'+i+'">'+
            '<option value="same_day">Same day</option>'+
            ...
            '<option value="6">6 days before</option>'+
            '<option value="7">7 days before</option>'+
            '</select></div><div style="height: 30px;"><input style="width:100% !important;" type="text" class="jqs-day-timepicker jqs-day-timepicker-'+i+'"></div></td>').
            appendTo($('.jqs-table tr.jqs-table-footer', this.element));
      }

      var schedule = this;

      $('.jqs-fillter-picker', this.element).on('change', function (event) {
        schedule.settings.onChangeMaxHour.call(schedule.element);
      });

      $('.jqs-day-timepicker', this.element).timepicker({
        timeFormat: 'HH:mm',
        interval: 30,
        minTime: '07:00',
        maxTime: '23:00',
      ...
      //$('.jqs-table').parent().find('.jqs-grid').css('width', $('.jqs-table').css('width'));
    },
     ...
     * @param args
     * @returns {Array}
     * TODO: add import for 'dates' type
     */
    import: function (args) {
      ...
      var ret = [];
      var daysTitles = [];
      $.each(dataImport, function (data_index, data) {
        ...
        if (data.max_hour) {
          ...
          $('.jqs-day-timepicker-'+ind, $this.element).val(data.max_hour);
        }
        // jqs-fillter-picker-
        if (data.filter_picker) {
          try {
            $('.jqs-fillter-picker-'+ind, $this.element).val(data.filter_picker);
          } catch (e) {
            console.log(e);
          }
        }
        $.each(data.periods, function (index, period) {
          var parent;
          if ($this.settings.type == 'weekly') {
            parent = $('.jqs-day', $this.element).eq(data.day);
          }
          else {
            parent = $('.jqs-day', $this.element).eq(data_index);
          }
          var options = {};
          var height, position;
          if ($.isArray(period)) {
        ...
        daysTitles.push(data.day);
      });

      if (this.settings.type == 'dates') {
        this.settings.daysList = daysTitles;
        this.updateSettingsDaysList(daysTitles);
      }
